...
**********************************************************************
	...
	Copyright (C)
2006 All rights reserved. Jacob Lee <letsgolee@lycos.co.kr>
...
function rteModeType(id)
{
	if (id == "rte_design_mode")
	{
		...
		document.getElementById(rteName).contentWindow.focus();return false;
	}
...
function rteBtnMouseDown()
{
	...
	rteSelection();this.className = "rtebtn4";
};
...
function rteBtnPrint()
{
	if (document.all)
	{
		...
		oFrame.focus();oFrame.print();
	}
	else
	{
		var oFrame = document.getElementById(rteName).contentWindow;
		oFrame.focus();oFrame.window.print();
	}
};
...
function rteInsertHTML(html)
{
	if (document.all)
	{
		var oRng = document.getElementById(rteName).contentWindow.document.selection.createRange();oRng.pasteHTML(html);oRng.collapse(false);oRng.select();
	}
	else 
	{
...
function rteFormHandler2()
{
	...
	matchesArray = newHTML.match(pattern);if (matchesArray != null)
	{
		for (i=0;i<matchesArray.length;i++)
		{
			pattern2 = /border=\"0\"/gi;
			replacement = matchesArray[i];
			replacement = replacement.replace(pattern2, "border=\"0\" class=\"rte_tbl\"");newHTML = newHTML.replace(matchesArray[i], replacement);
		}
	}
	if (document.getElementById(rteFormName).style.display == "")
	{
